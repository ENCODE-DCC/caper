...
def get_main_parser():
    ...
    parent_runner = parent_runner.add_argument_group(
        title='Cromwell logging arguments')
    parent_runner.add_argument('--cromwell-stdout',
    ...
    group_mysql = parent_runner.add_argument_group(
    ...
    group_postgresql = parent_runner.add_argument_group(
    ...
    group_cromwell = parent_runner.add_argument_group(
    ...
    group_local = parent_runner.add_argument_group(
    ...
    group_gc.add_argument('--gcp-prj',
        help='GC project')
    group_gc.add_argument('--gcp-zones',
    ...
    group_gc.add_argument(
        '--out-gcs-bucket',
        help='Output directory path for GCP backend. '
             'e.g. gs://my-bucket/my-output.')

    group_aws = parent_runner.add_argument_group(
    ...
    p_server = subparser.add_parser(
        ...
        parents=[parent_all, parent_server_client, parent_server, parent_runner,
                 parent_backend])
    p_submit = subparser.add_parser(
        'submit',
        help='Submit a workflow to a Cromwell server',
        parents=[parent_all, parent_server_client, parent_client, parent_submit,
                 parent_backend])
    ...
    p_troubleshoot = subparser.add_parser(
        ...
        help='Troubleshoot workflow problems from metadata JSON file or '
             'workflow IDs',
        parents=[parent_all, parent_server_client, parent_client, parent_search_wf, parent_troubleshoot])
