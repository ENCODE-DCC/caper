...
1) SLURM: 1 cpu, 6GB of RAM and 7 days of walltime. `--partition` (e.g. Stanford Sherlock) and `--account` (e.g. Stanford SCG) are optional and depend on your cluster's SLURM configuration.
	...
	```bash
	$ sbatch --export=ALL -n 1 --mem 6G -t 7-0 --partition [YOUR_SLURM_PARTITON] --account [YOUR_SLURM_ACCOUNT] --wrap "caper server --port 8000 [YOUR_CAPER_SERVER_EXTRA_PARAMS]"
	...
	```bash
	[leepc12@sh-ln07 login ~]$ sbatch --export=ALL -n 1 --mem 6G -t 7-0 -p akundaje -o ~/caper_server.o -e ~/caper_server.e --wrap "caper server --port 8000"
...
2) SGE: 1 cpu, 2GB of RAM and 7 days of walltime.
	...
	```bash
	$ echo "caper server --port 8000 [YOUR_CAPER_SERVER_EXTRA_PARAMS] --file-db [YOUR_FILE_DB]" | qsub -l h_rt=144:00:00 -l h_vmem=6G
